...
noncomputable instance KformulaCLK {agents : Type} [hN : fintype agents] : Kformula agents (formCLK agents) :=
{ knows := formCLK.K,
  everyone_knows := λ s, by classical; exact formCLK.E s.to_finset,
  K := @axCLK.K agents,
  T := @axCLK.T agents,
