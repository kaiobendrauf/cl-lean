...
def Bot {agents : Type} {form : Type} [pf : Pformula_ax form] [clf : CLformula agents form] := clf.Bot
def Top {agents : Type} {form : Type} [pf : Pformula_ax form] [clf : CLformula agents form] := clf.Top
def N   {agents : Type} {form : Type} [pf : Pformula_ax form] [clf : CLformula agents form] := clf.N
def M   {agents : Type} {form : Type} [pf : Pformula_ax form] [clf : CLformula agents form] := clf.M
def S   {agents : Type} {form : Type} [pf : Pformula_ax form] [clf : CLformula agents form] := clf.S
def Eq  {agents : Type} {form : Type} [pf : Pformula_ax form] [clf : CLformula agents form] := clf.Eq
...
def K    {agents : Type} {form : Type} [pf : Pformula_ax form] [kf : Kformula agents form] := kf.K
def T    {agents : Type} {form : Type} [pf : Pformula_ax form] [kf : Kformula agents form] := kf.T
def Four {agents : Type} {form : Type} [pf : Pformula_ax form] [kf : Kformula agents form] := kf.Four
def Five {agents : Type} {form : Type} [pf : Pformula_ax form] [kf : Kformula agents form] := kf.Five
def RN   {agents : Type} {form : Type} [pf : Pformula_ax form] [kf : Kformula agents form] := kf.RN

notation `K'` : 77         := Kformula.knows
notation `E'` : 77         := λ G φ, (finite_conjunction (list.map (λ i, K' i φ) (finset.to_list (finite.to_finset (finite.of_fintype G)))))
...
def C  {agents : Type} {form : Type} [hN : fintype agents] [pf : Pformula_ax form] [kf : Kformula agents form] [cf : Cformula agents form] := cf.C
def RC {agents : Type} {form : Type} [hN : fintype agents] [pf : Pformula_ax form] [kf : Kformula agents form] [cf : Cformula agents form] := cf.RC
