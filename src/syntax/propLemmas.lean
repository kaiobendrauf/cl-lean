...
-- ⊢ (ψ → χ) → ((φ → ψ) → (φ → χ))
 ax ((ψ →' χ) →' ((φ →' ψ) →' (φ →' χ))) :=
begin
exact (mp _ _ (mp _ _ (p2 _ _ _) (mp _ _ (p1 _ _) (p2 _ _ _))) (p1 _ _))
end
...
-- ⊢ φ → ((φ → ψ) → ψ)
 ax (φ →' ((φ →' ψ) →' ψ)) :=
begin
exact (mp _ _ (mp _ _ hs1 (mp _ _ (p2 _ _ _) iden)) (p1 _ _))
end
...
begin
intro h1,
exact mp _ _ (mp _ _ (p2 _ _ _) (mp _ _ (p1 _ _) (p1 _ _))) h1,
end
...
begin
intros h1 h2,
exact cut h1 (mp _ _ (p2 _ _ _) (mp _ _ (p1 _ _ )h2))
end
...
begin
intro h1,
exact mp _ _ (mp _ _ (p2 _ _ _ ) (mp _ _ (p1 _ _) (mp _ _ (p2 _ _ _) h1))) (p1 _ _)
end
...
ax ((φ →' (ψ →' χ)) →' (ψ →' (φ →' χ))) :=
begin
exact (mp _ _ (mp _ _ (p2 _ _ _) (cut (p2 _ _ _) hs1)) (mp _ _ (p1 _ _) (p1 _ _)))
end
...
-- ⊢ (φ → (φ → ψ)) → (φ → ψ)
 ax ((φ →' (φ →' ψ)) →' (φ →' ψ)) :=
begin
exact mp _ _ (p2 _ _ _) (imp_switch iden)
end
...
begin
intro h1,
exact mp _ _ double_imp (cut (p6 _ _) h1)
end
...
begin
exact mp _ _ (mp _ _ (p3 _ _) (cut dne (p6 _ _))) (cut dne (p5 _ _))
end
