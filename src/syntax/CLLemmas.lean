...
def ax_M' {agents: Type} {form: Type} [ft: formula form] [clf: CLformula agents form]
(φ ψ: form) (G: set agents):
-- ⊢ [G] (φ ∧ ψ) → [G] ψ
   ax (([G]' (φ ∧' ψ)) →' ([G]' ψ)) :=
begin
   have hab: ax (([G]' (φ ∧' ψ)) →' ([G]' (ψ ∧' φ))), from ef_and_switch,
   have hbc: ax (([G]' (ψ ∧' φ)) →' ([G]' ψ)), from (M _ _ _),
...
begin
   ...
   have h2: ax (φ →' (φ ∧' ψ)), from 
      imp_and_iden h,
   -- ⊢ ((φ ∧ ψ) → φ), from ⊤ by propositional logic
   have hp5: ax ((φ ∧' ψ) →' φ), from 
      (p5 _  _),
   ...
   have heq: ax (([G]'(φ ∧' ψ)) ↔' ([G]' φ)), from 
      (Eq _ _ _) hiff,
   -- ⊢ [G]φ → [G](φ ∧ ψ), from heq, by propositional logic
   have hif: ax (([G]' φ) →' ([G]' (φ ∧' ψ))), from 
      iff_r heq,
   -- ⊢ [G](φ ∧ ψ) → [G](ψ), by axiom (M)
   have hM:  ax (([G]'(φ ∧' ψ)) →' ([G]' ψ)), from 
      ax_M' φ ψ G,
...
def univ_iff_empty {agents: Type} {form: Type} [ft: formula form] [clf: CLformula agents form]
{φ: form}: 
   -- ⊢ [N] φ ↔ ⊢ ¬ [∅] ¬φ
   ...
   have hl: ax (([(univ : set agents)]' φ) →' (¬' ([(∅ : set agents)]' (¬' φ)))), from
      begin
         ...
         have hGiff: ax (([(univ: set agents)]' (φ ∧' (¬' φ))) ↔' ([(univ: set agents)]' ft.bot)),
             { apply Eq,
      ...
      -- ⇐ by the N axiom
   have hr: ax ((¬' ([(∅ : set agents)]' (¬' φ))) →' ([(univ : set agents)]' φ)), 
      from N _,
