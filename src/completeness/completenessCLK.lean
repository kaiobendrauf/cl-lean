...
-- def cl (φ : formCLK agents) : set (formCLK agents) 
--   -- | bot           := {(bot: formCLK agents)}
--   -- | (var n)       := s ∈ m.v n
--   | (imp ψ χ)     := {(imp φ ψ)} ∪ (cl φ) ∪ (cl ψ)
--   -- | (and ψ χ)     := (s_entails m s φ) ∧ (s_entails m s ψ)
--   -- | ([G] ψ)       := {t: m.f.states | s_entails m t φ} ∈ m.f.E.E (s) (G)
--   -- | (K' i ψ)      := ∀ t: m.f.states, t ∈ (m.f.rel i s) → s_entails m t φ
--   | ψ             := {ψ}
-- -- :=
-- --
{ -- --   sorry
