...
def emptyset_subset_univ {α: Type} (h: nonempty α): ∅ ⊂ @univ (α) :=
...
def intersect_complement {α: Type} (X Y: set α): (X ∩ Y)ᶜ ∩ X = Yᶜ ∩ X :=
...
def playable_from_semi_Nmax_reg (semi: semi_playable_effectivity_fun agents) (hNmax: N_max semi.f) (hReg: regular semi.f): playable_effectivity_fun agents :=
  ...
  begin
    ...
    {
      rw h,
      have hS: univ ∈ semi.f.E (s) (∅), from semi.semi_safety s ∅ (emptyset_subset_univ semi.f.ha),
  ...
  begin
    ...
    {
      rw h,
      have h0: ∅ ∉ semi.f.E (s) (∅), from semi.semi_liveness s ∅ (emptyset_subset_univ semi.f.ha),
  ...
  begin
    ...
    {exact semi.semi_monoticity s G X Y h hXY hX,},
    {
      ...
      have hmono: Yᶜ ∈ semi.f.E s ∅ → Xᶜ ∈ semi.f.E s ∅, from semi.semi_monoticity s ∅ Yᶜ Xᶜ (emptyset_subset_univ semi.f.ha) hXYc,
  ...
  -- have hcaseN: ∀ s: semi.f.states, ∀ X Y: set semi.f.states, X ∈ semi.f.E (s) (univ) → ∅ ∈ semi.f.E (s) (∅) → univ ∩ ∅ = ∅ → X ∩ Y ∈ semi.f.E (s) (univ ∪ ∅), from 
  -- begin
  --   intros s X Y hX hY hInt,
  --   -- Xᶜ ∉ f.E (s) (∅) → X ∈ f.E (s) (univ)
  ...
  --   have hIntc': _, from semi.semi_superadd s ∅ ∅ (X ∩ Y)ᶜ X (empty_union_subset_univ semi.f.ha) hIntc hX (inter_self ∅),
  --   -- simp at *,
  --   rw (intersect_complement X Y) at *,
  --   simp [union_self] at *,
    ...
    {exact semi.semi_superadd s G F X Y h hX hY hInt,},
    {
      ...
      cases (ssubset_or_eq_of_subset (subset_univ (G))), 
      {
        ...
        },
        {
          ...
          rw (intersect_complement X Y) at *,
          simp [union_self] at *,
          -- have hYc: _, from semi_playable_effectivity_fun.semi_monoticity s,
        },
      },
      {
        cases (ssubset_or_eq_of_subset (subset_univ (F))),
